...
中文文档【https://zh-hans.eslint.org/docs/latest/rules/getter-return】
...
npx eslint --init 生成eslint文件
配合编辑器去使用ESlint
以 VSCode 为例，在 VSCode 上使用 ESLint 需要安装 ESLint 插件。启用插件后，可以在编辑代码的同时看到哪些代码有问题，及时发现，及时修复。
编辑器的ESlint是如何帮助我们去检查的？
编写代码时候，编辑器自动执行eslint命令分析代码，并将结果返回到编辑器中
```
.eslintignore
此文件表明哪些路径应该忽略检测
/build/
 ...
 */ 
版权声明：本文为CSDN博主「小小亮01」的原创文章，遵循CC 4.0 BY-SA版权协议，转载请附上原文出处链接及本声明。
原文链接：https://blog.csdn.net/weixin_43845137/article/details/126681379
...
itorconfig 自定义文件是用来定义编辑器的编码格式规范，编辑器的行为会与 .editorconfig 文件中定义的一致，并且其优先级比编辑器自身的设置要高。
所以 EditorConfig 是用来帮助我们在不同的编辑器中保持编码风格的统一的。
````
参考文档：https://learn.microsoft.com/zh-cn/visualstudio/ide/create-portable-custom-editor-options?view=vs-2022
支持的属性
...
prettier的作用就是代码格式化，可以理解为prettier把代码解析之后再按照预期的规则进行重新打印，并且支持各种语言，如JavaScript、 Flow、 TypeScript、 CSS、 SCSS、 Less、 JSX、 Vue、 GraphQL、 JSON、 Markdown等等
.prettierrc.js
